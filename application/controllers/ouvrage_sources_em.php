...
//	// generate pagination
        ...
        foreach ($ouvrages as $source) {
            ...
            $this->table->add_row(++$i, anchor('entreprise/view/' . $entreprise->code_entreprise, strtoupper($entreprise->nom_de_l_entreprise), array()), anchor('projet/view/' . $projet->code_projet, strtoupper($projet->libelle_du_projet), array()), anchor('localite/view/' . $localite->code_de_la_localite, strtoupper($localite->nom), array()), $ouvrage->date_de_realisation, anchor('ouvrage_sources_em/view/' . $source->code_de_l_ouvrage, 'View', array('class' => 'view')) . ' ' .
                    anchor('ouvrage_sources_em/update_phaseOne/' . $source->code_de_l_ouvrage, 'Update', array('class' => 'update')) . ' ' .
                    anchor('ouvrage_sources_em/delete/' . $source->code_de_l_ouvrage, 'Delete', array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete this ouvrage ?')"))
            );
        }
...
//
        ...
        $source = $this->model->get_by_id('source_amenagees', $id, "code_de_l_ouvrage")->row();
...
//                    'code_de_la_localite' => $this->input->post('code_de_la_localite'),
                ...
                $source = $this->model->getEntity("SELECT * FROM source_amenagees WHERE code_de_l_ouvrage=" . $id . ";")->row();
                $this->model->update('source_amenagees', 'code_source_amenage', $source->code_source_amenage, $region);
