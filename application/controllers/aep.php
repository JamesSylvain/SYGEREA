...
//	// generate pagination
        ...
        foreach ($ouvrages as $source) {
            ...
            $this->table->add_row(++$i, anchor('entreprise/view/' . $entreprise->code_entreprise, strtoupper($entreprise->nom_de_l_entreprise), array()), 
                    ...
                    $ouvrage->date_de_realisation, anchor('aep/view/' . $source->code_de_l_ouvrage, 'View', array('class' => 'view')) . ' ' .
                    anchor('aep/update/' . $source->code_de_l_ouvrage, 'Update', array('class' => 'update')) . ' ' .
                    anchor('aep/delete/' . $source->code_de_l_ouvrage, 'Delete', array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete this ouvrage ?')"))
            );
        }
...
//	$this->template->layout('sidebar_ouvrage', 'welcome_message', $data);
        $this->template->layout('sidebar_ouvrage', 'aep/ouvrageListAep', $data);
    }
...
//
        ...
        $source = $this->model->get_by_id('aep', $id, "code_de_l_ouvrage")->row();
        ...
        $data['title'] = 'Modifier l\'ouvrage : Aduction en Eaux Potables';
        //		$data['message'] = '';
        $data['action'] = site_url('aep/update/' . $id);
        $data['link_back'] = anchor('aep/', 'Back to list of projet', array('class' => 'back'));
...
//
            ...
            $this->form_validation->set_rules('diametre', 'diametre de perforation', 'trim|required');
...
//            $this->model->update('ouvrage', 'code_de_l_ouvrage', $id, $region);
            ...
            $source = $this->model->getEntity("SELECT * FROM aep WHERE code_de_l_ouvrage=" . $id . ";")->row();
            $this->model->update('aep', 'code_aep', $source->code_aep, $region);
        ...
        $this->template->layout('sidebar_ouvrage', 'aep/ouvrageUpdateAep', $data);
    }
    ...
    function add() {
        ...
        $data['entreprises'] = $this->model->list_all('entreprise')->result();
        $data['localites'] = $this->model->list_all('localites')->result();
        $data['title'] = 'Création de l\'ouvrage : Aduction en Eaux Potables';
        //		$data['message'] = '';
...
////            $this->form_validation->set_rules('type_de_porosite', 'type de porosite', 'trim|required');
        ...
        $this->template->layout('sidebar_ouvrage', 'aep/ouvrageEditAep', $data);
    }

    function view($id) {
// set common properties
        $data['$id_aep']='submenu-active';
        $data['title'] = ' Details : Aduction en Eaux Potables';
        ...
        // load view
        $this->template->layout('sidebar_ouvrage', 'aep/ouvrageViewAep', $data);
    }
