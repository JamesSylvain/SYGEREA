...
//	// generate pagination
        ...
        foreach ($ouvrages as $ouvrage) {
            ...
            $this->table->add_row(++$i, anchor('entreprise/view/' . $entreprise->code_entreprise, strtoupper($entreprise->nom_de_l_entreprise), array()), anchor('projet/view/' . $projet->code_projet, strtoupper($projet->libelle_du_projet), array()), $ouvrage->population_desservie, $ouvrage->date_de_realisation, anchor('forages_ou_puits/view/' . $ouvrage->code_de_l_ouvrage, 'View', array('class' => 'view')) . ' ' .
                    anchor('forages_ou_puits/update/' . $ouvrage->code_de_l_ouvrage, 'Update', array('class' => 'update')) . ' ' .
                    anchor('forages_ou_puits/delete/' . $ouvrage->code_de_l_ouvrage, 'Delete', array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete this ouvrage ?')"))
            );
        }
...
//	$this->template->layout('sidebar_default', 'welcome_message', $data);
        $this->template->layout('sidebar_default', 'forages_ou_puits/ouvrageList', $data);
    }

    function create_hydrau() {
        $this->session->set_userdata('table_base', 'forages_ou_puits');
    ...
    function update($id = 0) {
        ...
        $ouvrage = $this->model->get_by_id('forages_ou_puits', $id, "code_de_l_ouvrage")->row();
        $projet = $this->model->getEntity('select projet.* from projet,ouvrage where projet.code_projet=ouvrage.code_projet and ouvrage.code_de_l_ouvrage=' . $ouvrage->code_de_l_ouvrage)->row();
        $entreprise = $this->model->getEntity('select entreprise.* from entreprise,ouvrage where entreprise.code_entreprise=ouvrage.code_entreprise and ouvrage.code_de_l_ouvrage=' . $ouvrage->code_de_l_ouvrage)->row();
        ...
        //		$data['message'] = '';
        $data['action'] = site_url('forages_ou_puits/update/' . $ouvrage->code_de_l_ouvrage);
...
//                    'code_projet' => $this->input->post('code_projet'),
                ...
                $this->model->update('forages_ou_puits', 'code_de_l_ouvrage', $id, $region);
                ...
                redirect('forages_ou_puits/index/');
            }
        } else {
        ...
        $this->template->layout('sidebar_default', 'forages_ou_puits/ouvrageUpdate', $data);
    }
...
//                    'code_projet' => $this->input->post('code_projet'),
    ...
    function view($id) {
        ...
        // load view
        $this->template->layout('sidebar_default', 'forages_ou_puits/ouvrageView', $data);
    }
